...
const LOGIN_PURPOSES = new Set(['primary', 'player2']);
...
export class AuthClient {
  ...
  async startLogin(returnTo = '/', purpose = 'primary') {
    if (!LOGIN_PURPOSES.has(purpose)) throw new AuthError(400, 'Invalid login purpose');
    ...
    if (purpose === 'player2') authorizeUrl.searchParams.set('prompt', 'select_account');
    return { authorizeUrl: authorizeUrl.toString(), state, purpose };
  }
  ...
  async completeLogin({ code, state, stateCookie }) {
    ...
    return {
      ...
      purpose: LOGIN_PURPOSES.has(transaction.purpose) ? transaction.purpose : 'primary',
    };
  }
