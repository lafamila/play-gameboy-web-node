...
export function createConfig(overrides = {}) {
  ...
  const config = {
    ...
    coreStorageDir: path.resolve(root, overrides.coreStorageDir ??
      process.env.CORE_STORAGE_DIR ?? 'core/dist'),
    ...
    player2SessionCookieName: overrides.player2SessionCookieName ??
      process.env.GBC_PORTING_PLAYER2_SESSION_COOKIE_NAME ?? 'gbc_porting_player2_session',
    oidcStateCookieName: overrides.oidcStateCookieName ??
      process.env.GBC_PORTING_OIDC_STATE_COOKIE_NAME ?? 'gbc_porting_oidc_state',
    player2OidcStateCookieName: overrides.player2OidcStateCookieName ??
      process.env.GBC_PORTING_PLAYER2_OIDC_STATE_COOKIE_NAME ?? 'gbc_porting_player2_oidc_state',
    ...
    localSessionLeaseSeconds: overrides.localSessionLeaseSeconds ?? integer(
      'LOCAL_2P_SESSION_LEASE_SECONDS',
      process.env.LOCAL_2P_SESSION_LEASE_SECONDS,
      90,
    ),
  ...
  const cookieNames = [
    ...
    config.player2SessionCookieName,
    config.oidcStateCookieName,
    config.player2OidcStateCookieName,
  ];
  if (cookieNames.some((name) => !/^[A-Za-z0-9_-]+$/.test(name)) ||
      new Set(cookieNames).size !== cookieNames.length) {
    throw new Error('P1/P2 session and OIDC state cookie names must be valid and distinct');
  }
