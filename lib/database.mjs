...
function normalizeLocalPayloads(payloads) {
  if (!Array.isArray(payloads) || payloads.length < 1 || payloads.length > 2) {
    throw databaseError('LOCAL_PAIR_INVALID', 'Local payloads must identify cartridge player slots');
  }
  const slots = new Set(payloads.map((entry) => entry?.slot));
  if (slots.size !== payloads.length || [...slots].some((slot) => slot !== 0 && slot !== 1)) {
    throw databaseError('LOCAL_PAIR_INVALID', 'Local payload slots must be unique slot 0 or 1 values');
  }
  return payloads;
}
...
function normalizeLinkParticipant(value, roomRomId) {
  if (typeof value === 'string') {
    return { accountId: value, romId: roomRomId, bootKind: 'cartridge' };
  }
  const bootKind = normalizeBootKind(value?.bootKind);
  if (bootKind === 'multiboot-client') {
    if (value?.romId !== undefined && value.romId !== null) {
      throw databaseError('MULTIBOOT_ROM_INVALID', 'Multiboot clients must not provide a ROM');
    }
    return { accountId: value?.accountId, romId: null, bootKind };
  }
  return { accountId: value?.accountId, romId: value?.romId ?? roomRomId, bootKind };
}

function normalizeBootKind(value = 'cartridge') {
  if (value !== 'cartridge' && value !== 'multiboot-client') {
    throw databaseError('BOOT_KIND_INVALID', 'bootKind must be cartridge or multiboot-client');
  }
  return value;
}

function normalizeLocalParticipant(participant) {
  const bootKind = normalizeBootKind(participant?.bootKind);
  if (bootKind === 'multiboot-client') {
    if (participant?.romId !== undefined && participant.romId !== null) {
      throw databaseError('MULTIBOOT_ROM_INVALID', 'Multiboot clients must not provide a ROM');
    }
    return { ...participant, romId: null, bootKind };
  }
  if (bootKind === 'cartridge' &&
      (typeof participant?.romId !== 'string' || participant.romId.length === 0)) {
    throw databaseError('ROM_NOT_FOUND', 'Cartridge participants require a ROM');
  }
  return { ...participant, bootKind };
}

function validateCartridgePayloads(participants, entries, code, message, identityKey) {
  const required = participants.filter((participant) => participant.bootKind === 'cartridge');
  const requiredIds = new Set(required.map((participant) => participant[identityKey]));
  const entryIds = new Set(entries.map((entry) => entry[identityKey]));
  if (entries.length !== required.length || entryIds.size !== required.length ||
      [...entryIds].some((id) => !requiredIds.has(id))) {
...
export class MariaDbDatabase {
  ...
  async migrate() {
    ...
    const statements = [
      ...
      `CREATE TABLE IF NOT EXISTS account_gamepad_mappings (
        account_id VARCHAR(128) NOT NULL,
        controller_key VARCHAR(255) NOT NULL,
        controller_label VARCHAR(255) NOT NULL,
        mapping_json TEXT NOT NULL,
        updated_at BIGINT UNSIGNED NOT NULL,
        PRIMARY KEY (account_id, controller_key)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
      ...
      `CREATE TABLE IF NOT EXISTS link_room_participants (
        ...
        account_id VARCHAR(128) NOT NULL,
        rom_id CHAR(64) NULL,
        boot_kind ENUM('cartridge', 'multiboot-client') NOT NULL DEFAULT 'cartridge',
        slot TINYINT UNSIGNED NOT NULL,
        ready BOOLEAN NOT NULL DEFAULT FALSE,
        connected BOOLEAN NOT NULL DEFAULT FALSE,
        save_revision BIGINT UNSIGNED NULL,
    ...
    await this.ensureColumn('link_rooms', 'game_group', 'VARCHAR(128) NULL AFTER `protocol_version`');
    await this.schemaExecutor.query(
      'ALTER TABLE link_rooms MODIFY COLUMN game_group VARCHAR(128) NULL',
    );
    ...
    await this.ensureColumn(
      ...
      'boot_kind',
      "ENUM('cartridge', 'multiboot-client') NOT NULL DEFAULT 'cartridge' AFTER `rom_id`",
    );
    await this.schemaExecutor.query(
      `UPDATE link_room_participants AS participants
       ...
       WHERE participants.rom_id IS NULL AND participants.boot_kind = 'cartridge'`,
    );
    await this.schemaExecutor.query(
      'ALTER TABLE link_room_participants MODIFY COLUMN rom_id CHAR(64) NULL',
    );
    await this.schemaExecutor.query(
      'ALTER TABLE link_room_participants MODIFY COLUMN save_revision BIGINT UNSIGNED NULL',
    );
    await this.ensureColumn(
      'local_link_participants',
      'boot_kind',
      "ENUM('cartridge', 'multiboot-client') NOT NULL DEFAULT 'cartridge' AFTER `rom_id`",
    );
    await this.schemaExecutor.query(
      'ALTER TABLE local_link_participants MODIFY COLUMN rom_id CHAR(64) NULL',
    );
    await this.schemaExecutor.query(
      'ALTER TABLE local_link_participants MODIFY COLUMN save_revision BIGINT UNSIGNED NULL',
    );
    ...
    await this.schemaExecutor.execute(
      `INSERT INTO schema_migrations (version, applied_at) VALUES (?, ?)
       ON DUPLICATE KEY UPDATE applied_at=applied_at`,
      ['gamepad-mapping-v1', Date.now()],
    );
    await this.schemaExecutor.execute(
      `INSERT INTO schema_migrations (version, applied_at) VALUES (?, ?)
       ON DUPLICATE KEY UPDATE applied_at=applied_at`,
      ['single-pak-v1', Date.now()],
    );
    } finally {
  ...
  async listGamepadMappings(accountId) {
    const [rows] = await this.pool.execute(
      `SELECT controller_key AS controllerKey, controller_label AS controllerLabel,
              mapping_json AS mappingJson, updated_at AS updatedAt
       FROM account_gamepad_mappings WHERE account_id = ? ORDER BY updated_at DESC`,
      [accountId],
    );
    return rows.map((row) => ({
      controllerKey: row.controllerKey,
      controllerLabel: row.controllerLabel,
      mapping: JSON.parse(row.mappingJson),
      updatedAt: Number(row.updatedAt),
    }));
  }

  async putGamepadMapping(accountId, value, now = Date.now()) {
    await this.pool.execute(
      `INSERT INTO account_gamepad_mappings (
         account_id, controller_key, controller_label, mapping_json, updated_at
       ) VALUES (?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE controller_label=VALUES(controller_label),
         mapping_json=VALUES(mapping_json), updated_at=VALUES(updated_at)`,
      [accountId, value.controllerKey, value.controllerLabel, JSON.stringify(value.mapping), now],
    );
    return { ...value, updatedAt: now };
  }

  async deleteGamepadMapping(accountId, controllerKey) {
    const [result] = await this.pool.execute(
      'DELETE FROM account_gamepad_mappings WHERE account_id = ? AND controller_key = ?',
      [accountId, controllerKey],
    );
    ...
    try {
      ...
      await connection.execute(
        `INSERT INTO link_room_participants (
           room_id, account_id, rom_id, boot_kind, slot, ready, connected,
  ...
  async joinLinkRoom(roomId, participantValue, now = Date.now()) {
    ...
    try {
      ...
      const [participants] = await connection.execute(
        `SELECT account_id AS accountId, boot_kind AS bootKind FROM link_room_participants
      ...
      const saveRevision = participant.bootKind === 'cartridge'
        ? await this.acquireLinkSaveLock(
      ...
      await connection.execute(
        ...
        [
          roomId, participant.accountId, participant.romId, participant.bootKind,
  ...
  async getLinkRoom(roomId) {
    ...
    const [participants] = await this.pool.execute(
      `SELECT account_id AS accountId, rom_id AS romId, boot_kind AS bootKind, slot,
    ...
    const [rows] = await this.pool.execute(
      `SELECT DISTINCT rooms.id, rooms.created_at AS createdAt
       ...
       WHERE (
         ...
         AND participants.account_id = ?
         AND (participants.rom_id = ? OR participants.boot_kind = 'multiboot-client')
  ...
  async putLinkCheckpoint(roomId, accountId, sequence, payload, now = Date.now()) {
    ...
    try {
      await connection.beginTransaction();
      const [participants] = await connection.execute(
        `SELECT account_id, boot_kind AS bootKind FROM link_room_participants
      ...
      if (participants[0].bootKind !== 'cartridge') {
        throw databaseError('LINK_CHECKPOINT_NOT_APPLICABLE', 'Multiboot clients do not persist checkpoints');
      }
      await connection.execute(
  ...
  validateLinkPair(participants, entries) {
    return validateCartridgePayloads(
      participants,
      entries,
      'LINK_PAIR_INVALID',
      'Payloads must contain exactly the cartridge room participants',
      'accountId',
    );
  }
  ...
  async getLinkCheckpointPair(roomId, sequence) {
    validateCheckpointSequence(sequence);
    const [participants] = await this.pool.execute(
      `SELECT account_id AS accountId, boot_kind AS bootKind
       FROM link_room_participants WHERE room_id = ? ORDER BY slot`,
      [roomId],
    );
    const expectedCount = participants.filter(
      (participant) => participant.bootKind === 'cartridge',
    ).length;
    const [rows] = await this.pool.execute(
    ...
    if (rows.length !== expectedCount || expectedCount === 0) return null;
    return { roomId, sequence, checkpoints: rows };
  }

  async getLatestLinkCheckpointPair(roomId) {
    const [rows] = await this.pool.execute(
      `SELECT DISTINCT checkpoint_sequence AS sequence FROM link_checkpoints WHERE room_id = ?
       ORDER BY checkpoint_sequence DESC`,
      [roomId],
    );
    for (const row of rows) {
      const checkpoint = await this.getLinkCheckpointPair(roomId, row.sequence);
    ...
    try {
      ...
      const [participants] = await connection.execute(
        `SELECT account_id AS accountId, rom_id AS romId,
                boot_kind AS bootKind, save_revision AS saveRevision
         FROM link_room_participants WHERE room_id = ? ORDER BY slot FOR UPDATE`,
        [roomId],
      );
      const cartridgeParticipants = this.validateLinkPair(participants, entries);
      const [locks] = await connection.execute(
      ...
      if (locks.length !== cartridgeParticipants.length) {
        throw databaseError('SAVE_LOCK_LOST', 'Link room no longer owns its cartridge save locks');
      }

      const lockByAccount = new Map(locks.map((lock) => [lock.accountId, lock]));
      committed = [];
      for (const participant of cartridgeParticipants) {
  ...
  async createLocalLinkSession(value) {
    ...
    if (participants[0].bootKind !== 'cartridge') {
      throw databaseError('BOOT_KIND_INVALID', 'Local slot 0 must boot from a cartridge');
    }
    const connection = await this.pool.getConnection();
    try {
      await connection.beginTransaction();
      ...
      for (const participant of participants) {
        ...
        await connection.execute(
          `INSERT INTO local_link_participants (
            session_id, slot, account_id, profile_key, rom_id, boot_kind, save_revision, ready
          ) VALUES (?, ?, ?, ?, ?, ?, ?, FALSE)`,
          [value.id, participant.slot, participant.accountId, participant.profileKey,
            participant.romId, participant.bootKind, revision],
        );
      }
      await connection.commit();
    } catch (error) {
  ...
  async putLocalCheckpointPair(id, sequence, payloads, metadata = {}, now = Date.now()) {
    ...
    try {
      ...
      const [participants] = await connection.execute(
        `SELECT slot, boot_kind AS bootKind FROM local_link_participants
         WHERE session_id = ? ORDER BY slot FOR UPDATE`,
        [id],
      );
      validateCartridgePayloads(
        participants,
        entries,
        'LOCAL_PAIR_INVALID',
        'Local payloads must contain exactly the cartridge player slots',
        'slot',
      );
      ...
      if (sequence === Number(sessions[0].lastCheckpointSequence)) {
        ...
        const matches = saved.length === entries.length && saved.every((entry) =>
  ...
  async getLatestLocalCheckpointPair(id) {
    ...
    const [rows] = await this.pool.execute(
      `SELECT checkpoint_sequence AS sequence FROM local_link_checkpoints
       WHERE session_id = ? GROUP BY checkpoint_sequence
       ORDER BY checkpoint_sequence DESC`,
      [id],
    );
    for (const row of rows) {
      ...
      if (expectedCount > 0 && checkpoints.length === expectedCount) {
        return { sessionId: id, sequence: row.sequence, checkpoints };
      }
    }
    return null;
  }
    ...
    try {
      ...
      const [participants] = await connection.execute(
        `SELECT slot, account_id AS accountId, profile_key AS profileKey,
                rom_id AS romId, boot_kind AS bootKind, save_revision AS saveRevision
      ...
      const cartridgeParticipants = validateCartridgePayloads(
        participants,
        entries,
        'LOCAL_PAIR_INVALID',
        'Local payloads must contain exactly the cartridge player slots',
...
export class MemoryDatabase {
  ...
  gamepadMappingKey(accountId, controllerKey) { return `${accountId}\0${controllerKey}`; }
  async listGamepadMappings(accountId) {
    return [...this.gamepadMappings.values()]
      .filter((value) => value.accountId === accountId)
      .sort((left, right) => right.updatedAt - left.updatedAt)
      .map(({ accountId: _accountId, ...value }) => structuredClone(value));
  }
  async putGamepadMapping(accountId, value, now = Date.now()) {
    const stored = { accountId, ...structuredClone(value), updatedAt: now };
    this.gamepadMappings.set(this.gamepadMappingKey(accountId, value.controllerKey), stored);
    const { accountId: _accountId, ...result } = stored;
    return structuredClone(result);
  }
  async deleteGamepadMapping(accountId, controllerKey) {
    return this.gamepadMappings.delete(this.gamepadMappingKey(accountId, controllerKey));
  }
  ...
  async joinLinkRoom(roomId, participantValue, now = Date.now()) {
    ...
    if (!room) throw databaseError('LINK_ROOM_NOT_FOUND', 'Link room not found');
    const participant = normalizeLinkParticipant(participantValue, room.romId);
    if (participant.bootKind === 'cartridge' && !this.roms.has(participant.romId)) {
      throw databaseError('ROM_NOT_FOUND', 'ROM not found');
    }
  ...
  async getRecoverableLinkRooms(accountId, romId) {
    ...
    for (const participant of this.linkParticipants.values()) {
      ...
      if (participant.accountId === accountId &&
        (participant.romId === romId || participant.bootKind === 'multiboot-client')
  ...
  async getLinkCheckpointPair(roomId, sequence) {
    ...
    const checkpoints = participants.flatMap((participant) => {
    ...
    return participants.length > 0 && checkpoints.length === participants.length
      ? { roomId, sequence, checkpoints }
      : null;
  }
  async getLatestLinkCheckpointPair(roomId) {
  ...
  async createLocalLinkSession(value) {
    ...
    try {
      for (const participant of participants) {
        if (participant.bootKind !== 'cartridge') continue;
    ...
    for (const { key, participant, revision } of locks) {
      ...
      revisionBySlot.set(participant.slot, revision);
    }
    for (const participant of participants) {
      this.localLinkParticipants.set(this.localParticipantKey(value.id, participant.slot), {
        sessionId: value.id,
        ...participant,
        saveRevision: revisionBySlot.get(participant.slot) ?? null,
        ready: false,
      });
    }
    return this.getLocalLinkSession(value.id);
  ...
  async getLatestLocalCheckpointPair(id) {
    const requiredSlots = this.localParticipantsForSession(id)
      .filter((participant) => participant.bootKind === 'cartridge')
    ...
    for (const sequence of [...sequences].sort((left, right) => right - left)) {
      const checkpoints = requiredSlots.flatMap((slot) => {
        const item = this.localLinkCheckpoints.get(this.localCheckpointKey(id, sequence, slot));
        return item ? [{ ...item, payload: Buffer.from(item.payload) }] : [];
      });
      if (requiredSlots.length > 0 && checkpoints.length === requiredSlots.length) {
        return { sessionId: id, sequence, checkpoints };
      }
    }
    return null;
  }
