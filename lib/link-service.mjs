...
export const LINK_CORE_VERSION = 'vba-link-1.72-web-v1';
export const LINK_PROTOCOL_VERSION = 'gba-cable-v1';

const POKEMON_GEN3_CODES = new Set(['BPR', 'BPG', 'BPE', 'AXV', 'AXP']);

export class LinkService extends EventEmitter {
  constructor({ database, coordinator = new LinkRoomCoordinator(), now = Date.now } = {}) {
    super();
  ...
  async joinRoom({ roomId, accountId, inviteCode, romId }) {
    ...
    if (persisted.expiresAt && persisted.expiresAt <= this.now()) {
      ...
      await this.database.updateLinkRoomStatus(roomId, 'aborted');
    ...
    try {
      ...
      await this.database.updateLinkRoomStatus(roomId, 'aborted').catch(() => {});
      throw error;
    }
  ...
  async setReady({ roomId, accountId, ready }) {
    ...
    await this.database.setLinkParticipantState(roomId, accountId, { ready: ready !== false });
    await this.database.updateLinkRoomStatus(roomId, snapshot.status);
  ...
  async startRoom({ roomId, accountId }) {
    ...
    await this.database.updateLinkRoomStatus(roomId, 'active');
    const room = await this.getRoom({ roomId, accountId });
    this.#emitRoom(roomId, null, { type: 'room', room });
    return room;
  }
  ...
  async disconnect({ roomId, accountId }) {
    ...
    try {
      ...
      if (error instanceof LinkRoomError && error.code === 'INVALID_STATE') return null;
      throw error;
    }
...
function transferPayload(message) {
  ...
  return { speed: message.speed, data: message.data };
}
