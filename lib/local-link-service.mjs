...
export class LocalLinkService {
  ...
  async create({
    ...
    const resolvedBootKind = readBootKind(player2BootKind ?? bootKind);
    ...
    if (resolvedBootKind === 'multiboot-client' &&
        player2RomId !== undefined && player2RomId !== null) {
      throw localError(400, 'MULTIBOOT_ROM_INVALID', 'Multiboot clients must not provide a ROM');
    }
    const player1Rom = await this.#gbaRom(player1RomId);
    const player2Rom = resolvedBootKind === 'cartridge'
      ? await this.#gbaRom(player2RomId)
      : null;
      ...
      participants: [
        ...
        player2Mode === 'account'
          ? {
              ...
              profileKey: 'primary',
              romId: player2Rom?.id ?? null,
              bootKind: resolvedBootKind,
            }
          : {
              slot: 1,
              accountId: player1.accountId,
  ...
  async checkpoint({ id, sequence, states, metadata, player1, player2 }) {
    ...
    const requiredSlots = cartridgeSlots(session);
    const payloads = decodePayloads(states, requiredSlots, 2 * 1024 * 1024, 'checkpoint');
  ...
  async finish({ id, batteries, player1, player2 }) {
    ...
    const requiredSlots = cartridgeSlots(session);
    const payloads = decodePayloads(batteries, requiredSlots, 131_072, 'battery');
...
function readBootKind(value = 'cartridge') {
  if (value !== 'cartridge' && value !== 'multiboot-client') {
    throw localError(400, 'BOOT_KIND_INVALID', 'bootKind must be cartridge or multiboot-client');
  }
  return value;
}

function cartridgeSlots(session) {
  return session.participants
    .filter((participant) => participant.bootKind === 'cartridge')
    .map((participant) => participant.slot);
}

function decodePayloads(values, requiredSlots, maxBytes, label) {
  if (!Array.isArray(values) || values.length !== requiredSlots.length) {
    throw localError(400, 'LOCAL_PAIR_INVALID', `Required ${label} payloads are missing`);
  }
  const slots = new Set(values.map((entry) => entry?.slot));
  if (slots.size !== requiredSlots.length || requiredSlots.some((slot) => !slots.has(slot))) {
    throw localError(400, 'LOCAL_PAIR_INVALID', `Invalid ${label} participant slots`);
  }
